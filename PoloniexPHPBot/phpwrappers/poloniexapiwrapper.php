.<?php
    ...
    //SOURCE: https://pastebin.com/iuezwGRZ
	...
	class poloniex {
		...
		public function get_my_trade_history($pair, $start) {
            $date=new DateTime();
			return $this->query(
				array(
		...
		public function get_total_btc_balance() {
			$balances = $this->get_complete_balances();
            
			//var_dump($balances[strtoupper('btc')]);
			...
			foreach($balances as $coin => $amount){
				...
				if($amount['btcValue'] > 0){
                    
					if($coin != "BTC" && $coin != "USDT"){
                        $tot_btc += $balances[strtoupper($coin)]['btcValue'];
                        
					}else{
                        if($coin != "USDT")
                        {
                            $tot_btc += $amount['btcValue'];
                        }
					}
				}
            }
